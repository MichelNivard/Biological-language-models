...
Recent advances in genomic language models (gLMs) have transformed our ability to analyze DNA as if it were a natural language, unlocking insights into regulatory mechanisms, variant effects, and even protein function. Here’s an overview of the state‐of‐the‐art:
...
While transformer-based models initially dominated the field, their quadratic scaling with sequence length has prompted the development of more efficient architectures. Models such as HyenaDNA extend context lengths up to 1 million tokens at single-nucleotide resolution, and hybrid architectures like HybriDNA combine transformers with selective state-space models (Mamba2) to process sequences up to 131 kilobases. Omni-DNA and GENERator further illustrate the trend toward unified, cross-modal genomic foundation models capable of multitask learning.

Below is a summary table of several prominent genomic language models along with links to their corresponding paper and GitHub (or related resource) repositories:

| Model | Description | Paper Link | GitHub / Resource Link |
|---------------|------------------------|-----------------|----------------|
| **DNABERT** | A transformer-based model that learns bidirectional representations from DNA sequences using k-mer tokenization. | [DNABERT Paper](https://doi.org/10.1093/bioinformatics/btab083) |  |
| **Nucleotide Transformer (NT‑v2)** | A large transformer pretrained on massive human genomic data to learn robust DNA representations for various downstream tasks. | [Nucleotide Transformer v2](https://doi.org/10.1038/s41592-024-02523-z) |  |
...
In summary, the field is rapidly evolving as researchers refine architectures and training strategies to capture the intricate, long-range dependencies inherent to the genome, promising to revolutionize our understanding of biological function and disease.