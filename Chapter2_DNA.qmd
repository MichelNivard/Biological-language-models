...
Because we used a standard BERT model (**`BertModern`**), it's super easy for others to pull the model weights from the hub into a model configured for use on their machine, using the Hugging Face **`Transformers`** library. If you'd want to train a sate of the art model you'd bviousnely need to scale training (see @sec-scale for guidance).
