...
In this chapter we'll train an DNA language model. To do a full training run you'll need newer Macbook (±24 hours), a gaming PC with GPU (10-12 hours) or google colab with the A100 GPU (±6 hours).
...
We'll briefly have to discuss the general architecture of the model, in bold the key elements of the model, in bold and italic parameters we get to set to etermin the size of the model.
...
The keenly eyed among you see a lot of powers of 2, 8 is 2^3^, 256, 512 are also powers of two etc. Computer memory encodes in bits and bytes, and is designed around powers of two. Buiding matrices that are powers of 2,16,32,64 etc makes them fit in memory more efficient and this can have serious consequences for training efficiency (see **Figure 2.1**).
...
Unlike traditional NLP tasks where models are trained on massive text corpora, training on DNA sequences presents unique challenges. The structure of genomic data, sequence length, and biological patterns all influence how the model learns. Our configuration is again rathr general and not yet tuned to DNA or biological data. If you would scale this model, you'd likely have to drop the learning rate down for example. In "production" hyper parameter optimization becomes super important when you train a large model on all your data each run might be costly and setting optimal hpyper parameters can lead to serious gains in training results.
...
-   **Training Steps:** The model is trained for `4000` steps (`max_steps`), though on the google cloab code I ran in the cloud I trained for 2 whole epochs over all data (**`num_train_epochs = 2`**) which is a more precise qay to ensure the model sees all the data twice.
-   **Model Saving:** The model checkpoints are stored in `./bert-dna`, allowing us to resume training from a checkpoint if needed (after a computer crash, or after the model going of the rails etc).
...
While I have enabled it here I can recommend tacking your training runs on wandb. you go to [wandb.ai](www.wandb.ai) (w and b meaning weights and biases, the core parameters in AI models) to make a free account. Now to some extend this is like strava but for AI, and there is a risk of obsessing over the training meta data. But if you find yourself with a limited amount of compute, or expensive compute paid per minute, it makes a lot of sense to track big training runs in real time so you can intervene. If the run crashes, you can restart, or abort the node so you aren't paying for an expensive GPU node tats no longer training.

### Preparing for Masked Language Modeling (MLM)

Since we are training our DNA BERT model using **masked language modeling (MLM)**, we need to handle instriducing masked tokens properly. This is done using the **`DataCollatorForLanguageModeling`**, which:
...
If you have set up free wandb account you can track your training runs, wherever they are running, on a central dashboard. You then get a dashboard full of pretty loss vs progress plots like the one below which I screencapped about ± 30 minutes into training a tiny version of the model on my macbook.
...
Because we used a standard Bert model (**`BertModern`**), its super easy for others to pull the model weights from the hub into a model configured for use on their machine, using the Hugginface **`Transformers`** library.
...
-   Used masked language modeling (MLM) to train the model to pedict gaps in DNA sequences.
