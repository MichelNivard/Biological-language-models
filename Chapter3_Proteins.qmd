...
::: callout-tip
Cheng et. all reveal something profound about model loss and scientits's goals. The loss isn't a direct measure of the modle's quality for your specific goals. If you are to design a protein language model for a specific tasks (predict the fold of human proteins after mutation for example) then you have to design validation accordingly. We read in this chapter, and last, that because we are running out of animal and plant reference protein data people include meta-genomic protein data is training. Thets probably a great idea, but its worth considering weather for YOUR downstream tasks it is. ESM3 is better then ESM2, is its trained on way more data, but is it better for *your* applications?
